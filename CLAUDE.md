...
| --- | --- | --- |
| Unit | Vitest, `app` project (node) | **All of `src/lib/`** — `mdt/codec`, `mdt/route`, `mdt/useRouteDoc`, `geometry`, `indicators`, `content`, `content.integrity`, `contribution`, `data`, `highlights`, `tips`, `tipIndex`, `collab/presence`, `i18n/detect`, `i18n/format` — plus `map/viewport`, `scripts/tile-layout`, `scripts/lua-table`, `scripts/mdt-dungeon`, `scripts/wowhead-tooltip` and `scripts/content-stub` |
| Integration | Vitest, `app` project (jsdom) | Every component — the codex chain (`Badges`, `MobCard`, `CodexPanel`, `MobTips`), the highlights chain (`MobTable`, `TrapList`, `BossStrip`, `TipList`, `TipCard`), `RoutePanel`, `DungeonMap`, the home page, `TipsIndex`, which lists the season's tips by dungeon, and `DungeonPage`, which mounts the map and both side panels together — against the real dungeon pool |
...
| End-to-end | Playwright, Chromium (`npm run test:e2e`) | Scenarios in a real browser: a real browser socket, carrying a real `Origin`, is accepted by the real relay and presence comes back (the deployed host's entry in `relay/src/index.js`'s `ALLOWED_ORIGINS` cannot be exercised from this local harness, and stays verified by eye); a join link carrying the deployed sub-path into a second browser, whose invitation is asserted **in the viewport** rather than merely visible — only a real browser can tell those apart, and the invitation used to be reachable only by scrolling; two viewports agreeing on where a shared cursor points; a local route set aside on joining and handed back on leaving; an eraser drag that survives the map's own pointer capture and retargeting; a stroke that reaches a second browser while it is still being drawn; a video tip's embed loading only once the reader clicks, with no request to YouTube before that click, and its link out surviving the deployed sub-path; the map marking the pull a scoped tip is about with a `?` on the pack itself — one mark, rather than a badge on each of the eleven clones of the mob the tip happens to be written on; the briefing page's own tip card loading no embed and contacting no YouTube host until its button is clicked, after which the iframe names the right video; a video folded away again dropping its embed and reopening on a third click; the briefing listing the three bosses Den of Nalorakk really has rather than the four MDT flags, with the demoted one marked in place on its own row; a tip's pull chip on the season index jumping the map to the pull it names, asserted by the blip growing well past double its unfocused size and its centre landing inside the map's visible area, since jsdom lays everything out at zero and leaves that arithmetic unobservable there — plus one smoke test that only proves the harness loads the build |
...
| [`codex-content`](.claude/skills/codex-content/SKILL.md) | writing or editing a `content/**.md` entry, rating a mob's `threat`, or reconciling MDT against another source. It is a pointer: the rules themselves live in [`docs/writing-cards.md`](docs/writing-cards.md), where a contributor with no agent can read them too, in either language |
