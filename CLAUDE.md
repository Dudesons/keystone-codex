...
| --- | --- | --- |
| Unit | Vitest, `app` project (node) | **All of `src/lib/`** — `mdt/codec`, `mdt/route`, `mdt/useRouteDoc`, `geometry`, `indicators`, `content`, `content.integrity`, `contribution`, `data`, `highlights`, `tips`, `collab/presence`, `i18n/detect`, `i18n/format` — plus `map/viewport`, `scripts/tile-layout`, `scripts/lua-table`, `scripts/mdt-dungeon`, `scripts/wowhead-tooltip` and `scripts/content-stub` |
| Integration | Vitest, `app` project (jsdom) | Every component — the codex chain (`Badges`, `MobCard`, `CodexPanel`, `MobTips`), the highlights chain (`MobTable`, `TrapList`, `BossStrip`, `TipList`), `RoutePanel`, `DungeonMap`, the home page, and `DungeonPage`, which mounts the map and both side panels together — against the real dungeon pool |
