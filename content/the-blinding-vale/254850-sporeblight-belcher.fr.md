---
npcId: 254850

# Traduction : seuls les champs de texte sont repris. `threat`, `role`, `tag` et `prio` sont
# des jugements, pas de la langue — ils restent dans la fiche de base et se fusionnent ici.

spells:
  - id: 1263636
    note: "291k Nature par impact, un toutes les 1,5 s pendant 3 s. Des dégâts au sol — toute l'incantation s'évite en bougeant."
  - id: 1263628
    note: "87k à tout le monde dans les 60 mètres toutes les 2 s pendant 6 s. La distance n'est pas une échappatoire — celle-là se soigne, elle ne s'esquive pas."
  - id: 1242200
    note: "291k à tous les proches à sa mort, et ça laisse du Blight Resin au sol. Onze corps comme celui-là, c'est onze explosions."

trap: "Vingt-cinq forces par corps et 291k à chaque mort. Ne pas en laisser plusieurs mourir sur le groupe en même temps — étaler les kills ou les disperser."

tips:
  - video: https://www.youtube.com/shorts/9D0gCU8Tp5Y
    label: "Naowh — prendre le pull après le premier boss"
    packs: [44]
---

**Le mob le plus lourd du donjon, et de loin** : onze unités à 25 forces chacune, environ 40 % du
compte qu'un groupe doit atteindre. Aucune route ne l'évite.

Deux de ses capacités sont l'échange habituel — **Éructation sporulée** est un dégât au sol
entièrement évitable, **Jaillissement floral** porte à 60 mètres et ne l'est pas. Celle qui décide
des pulls est la troisième.

**Chancre de gardelumière** n'est pas son œuvre : c'est la marque du Gardelumière, portée par la
moitié du trash de The Blinding Vale, et elle détonne pour 291k quand le corps tombe. Sur un pull
qui tient trois ou quatre Cracheurs, le danger n'est pas le combat — c'est le moment où ils
meurent tous ensemble.
