...
function VideoTip({ tip }: { tip: Extract<Tip, { kind: 'video' }> }) {
  ...
  return (
    <div>
      {/* The row stays put while the embed is open, rather than being replaced by it: a Short is
          tall, and a reader who opens one needs something left on screen to close it with. */}
      <div className="flex items-center gap-2">
        <button
          onClick={() => setPlaying((open) => !open)}
          aria-expanded={playing}
          title={playing ? t('tip.collapse') : t('tip.play')}
      ...
      {/* Unmounted when folded, not hidden. This section is built on nothing reaching Google
          until the reader asks: a player left alive behind a folded row would hold the
          connection the fold was meant to end, and keep playing its audio while it did. */}
      {playing && (
        <div
          className={`mt-1.5 overflow-hidden rounded border border-ink-700 ${
