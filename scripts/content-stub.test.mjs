// ABOUTME: Tests the markdown stub builder and the scaffold plan.
// ABOUTME: Pins the invariant that matters: an existing card is never overwritten.
...
/**
 * `src/lib/content.test.ts` needs an untouched scaffold stub to parse, and the codex is now
 * fully written, so no real one is left. The fixture it uses is committed under
 * `content/__fixtures__/` and generated by this very builder from the real MDT file above.
 *
 * This test is what keeps it honest: if the scaffold template changes and the committed stub
 * is not regenerated, the fixture stops representing what `npm run scaffold` produces and the
 * parsing tests would be validating a shape the repository no longer emits.
 */
describe('The committed content fixture', () => {
  const path = fileURLToPath(new URL('../content/__fixtures__/270306-ritual-chieftain.md', import.meta.url))
  const realSpells = JSON.parse(
    fs.readFileSync(fileURLToPath(new URL('../src/data/generated/spells.json', import.meta.url)), 'utf8'),
  )

  it('is exactly what buildMobStub produces today', () => {
    expect(fs.readFileSync(path, 'utf8')).toBe(buildMobStub(chieftain, realSpells))
  })
})
